...
This code explores data on red and purple sea urchins recorded by Santa Barbara Coastal Long Term Ecological Research program divers. It looks at the distribution of red and purple urchins at 5 sites from 2008 to 2020, which we narrow down to look at the change of purple urchin population. 
...
urchin_summary %>% 
  kable(digits = 2, # in kable() can use digits = x to round the values in the table - can use a c("x","y") to define the number of digits for each column
     ...
     kable_styling(bootstrap_options = "striped", full_width = F, position = "center") # bootstrap will make the table look all nice with different colors, I like it
...
ggplot(data = purple_count,
       aes(x = year, y = count)) +
  geom_col(aes(fill = site_full)) + # chose to use a column graph because it shows the trend in a very visible, discrete way
  ...
  labs(fill = "Site",
       ...
       caption = "Fig 2. Purple urchin counts at each observation site froom 2008 to 2020 based on Santa Barbara Coastal Long \n Term Ecological Research") +
